# DataWalk_2.0.1.py
# Run: python DataWalk.2.py
# To make executable:
# Install pyinstaller: pip install pyinstaller
# Run: pyinstaller --onefile DataWalk_2.0.1.py
...
# 4. Run: python DataWalk_2.0.1.py
# To find your kill: ps aux | grep python
# Kill top running: kill -9 PID#
# Kill run-away intances (PID keeps chaning but all stable PIDs of grep list must be killed first): pkill -f DataWalk_2.0.1.py
import sys
import os
import math
import colorsys
...
class DataWalk(ShowBase):
    def __init__(self):
        ...
        self.shadow.setPythonTag("vdm", True)
        self.shadow.setPythonTag("type", "player_shadow")
        self.shadow.setCollideMask(BitMask32.bit(0))
        self.shadow.setZ(0.1)
    ...
    def open_selected(self):
        ...
        if typ == "player_shadow":
            ...
            return
        if os.path.isdir(path) and typ != "player_shadow":
            self.interact()
            self.toggle_beacon()
        else:
            try:
    ...
    def pause_game(self):
        ...
        self.player_height_slider = DirectSlider(parent=canvas, range=(5, 13), value=self.player_height, pos=(0.2, 0, y_pos), scale=0.8, command=self.update_player_height)
        y_pos -= 0.15
        DirectLabel(parent=canvas, text="Z-Axis Multiplier:", pos=(-1.1, 0, y_pos), scale=0.07, text_align=TextNode.ALeft)
        self.z_multiplier_slider = DirectSlider(parent=canvas, range=(1.7, 5.3), value=self.z_multiplier, pos=(0.2, 0, y_pos), scale=0.8, command=self.update_z_multiplier)
        y_pos -= 0.15
        ...
        y_pos -= 0.15
        DirectLabel(parent=canvas, text="Hidden Files:", pos=(-1.1, 0, y_pos), scale=0.07, text_align=TextNode.ALeft)
    ...
    def toggle_screen(self):
        ...
        if hasattr(self, 'screen_slider'):
            self.screen_slider.setValue(self.screen_mode)
        self.save_config()
    ...
    def load_path(self, path):
        ...
        for child in self.render.getChildren():
            if child.hasPythonTag("vdm") and child != self.shadow:
                child.removeNode()
        ...
        for i, name in enumerate(non_hidden):
            ...
            if os.path.isdir(full_path):
                ...
                self.create_file_orb(x, y, 0, name, full_path, size)
        if self.show_hidden:
    ...
    def create_building(self, x, y, name, path, file_count, dir_count, files):
        ...
        cube = self.make_cube(building, base_width, base_depth, height, self.building_color)
    ...
    def update_camera(self, task):
            ...
            if hit_node and hit_node.hasPythonTag("type"):
                ...
                if typ in ["exit_ground", "ground"]:
                    ...
                    if pitch <= -14:
                        pause_angle = 3
                        no_xy_angle = 6
                        if angle < pause_angle:
                            z_delta = 0
                            no_xy = True
                        else:
                            z_delta = -self.z_speed * dt
                            if angle < no_xy_angle:
                                no_xy = True
        if pitch >= 14:
            pause_pitch = 87
            no_xy_pitch = 84
        ...
        if new_label_at_top != self.label_at_top:
            ...
            self.update_label_positions()
        self.shadow.setX(self.camera.getX())
        self.shadow.setY(self.camera.getY())
        return Task.cont
